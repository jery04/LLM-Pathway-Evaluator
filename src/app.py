...
def main():
    ...
    if submitted:
        ...
        goal = goal or (user_text.strip() if user_text and user_text.strip() else 'Data Scientist')
        ...
        with st.spinner('Generating paths...'):
            knows_python = any('python' in skill.lower() for skill in (initial_skills + parser_skills))
        ...
        if not paths:
            ...
            try:
                explanation = explain_comparison(paths, {'goal': goal, 'skills': initial_skills, 'objective': goal, 'preferences': preferences})
            except Exception:
                explanation = ''
